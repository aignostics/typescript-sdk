...
function getDefaultAuthService(): AuthService {
  if (!defaultAuthService) {
    defaultAuthService = new AuthService(new FileSystemTokenStorage());
  }
...
/**
 * Perform OAuth2 PKCE login flow with external callback handling
 * @deprecated Use AuthService class directly for better control
 */
export async function loginWithCallback(config: LoginWithCallbackConfig): Promise<string> {
  return getDefaultAuthService().loginWithCallback(config);
}
...
export async function completeLogin(
  ...
  return getDefaultAuthService().completeLogin(config, authCode);
}
...
export async function getValidAccessToken(): Promise<string | null> {
  return getDefaultAuthService().getValidAccessToken();
}
...
export async function getAuthState(): Promise<AuthState> {
  return getDefaultAuthService().getAuthState();
}
...
export async function login(config: LoginConfig): Promise<void> {
  return getDefaultAuthService().login(config);
}
